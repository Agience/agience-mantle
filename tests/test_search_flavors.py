...
from mantle.services import search_flavors


class _Ctx:
    def __init__(self, user_id="user-1"):
        self.user_id = user_id
        self.store_db = object()  # truthy; gate path is monkeypatched
...
def test_premium_flavor_denied_without_entitlement(monkeypatch):
    monkeypatch.setattr("mantle.services.gate_service.has_feature", lambda db, uid, feat: False)
    art = {"context": {"run": {
...
def test_premium_entitled_but_server_unregistered_degrades_503(monkeypatch):
    monkeypatch.setattr("mantle.services.gate_service.has_feature", lambda db, uid, feat: True)
