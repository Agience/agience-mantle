...
from mantle.search.mantle.engine import MantleHit, MantleQueryEngine
...
from mantle.search.mantle.sse.unified import _collapse_mantle_hits_to_artifact, _rrf_fuse
...
class TestUnifiedAccessor:
    ...
    def test_empty_contexts_returns_empty(self, sse_engine):
        ...
        assert acc.search("anything", [], req()) == []

    def test_top_k_zero(self, sse_engine):
        acc = MantleUnifiedAccessor(sse_engine)
        assert acc.search("anything", [("owner-A", "col-1")], req(), top_k=0) == []
    ...
    def test_sse_only_when_no_query_embedding(
        ...
        hits = acc.search("encryption", [("owner-A", "col-1")], req())
        assert len(hits) == 1
    ...
    def test_top_k_truncates(self, sse_engine, indexer):
        ...
        acc = MantleUnifiedAccessor(sse_engine)
        hits = acc.search(
            "library", [("owner-A", "col-1")], req(), top_k=1)
        assert len(hits) == 1
    ...
    def test_descending_order_by_rrf_score(self, sse_engine, indexer):
        ...
        acc = MantleUnifiedAccessor(sse_engine)
        hits = acc.search(
            "alpha beta gamma", [("owner-A", "col-1")], req())
    ...
    def test_no_matches_anywhere_returns_empty(self, sse_engine, indexer):
        ...
        self_request("owner-A", "update"))
        acc = MantleUnifiedAccessor(sse_engine)
        hits = acc.search("zebra", [("owner-A", "col-1")], req())
        assert hits == []
