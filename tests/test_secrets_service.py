...
get_secret_value). All DB writes are intercepted at lattice_identity_legacy.
"""
...
import mantle.services.secrets_service as ss
...
def _reset_cipher():
    ...
    with patch("mantle.services.secrets_service.get_encryption_key", return_value=key):
        yield
    ss._cipher = None
...
class TestEncryptDecrypt:
    ...
    def test_unconfigured_key_raises_runtime_error(self):
        ss._cipher = None
        with patch("mantle.services.secrets_service.get_encryption_key", return_value=None):
...
class TestSecretsCrud:
    def test_list_filters_by_type_and_provider(self):
        ...
        with patch("mantle.services.secrets_service.identity_store.get_person_by_id", return_value=prefs):
    ...
    def test_list_returns_empty_when_no_person(self):
        with patch("mantle.services.secrets_service.identity_store.get_person_by_id", return_value=None):
    ...
    def test_add_secret_encrypts_and_persists(self):
        ...
        with (
            patch("mantle.services.secrets_service.identity_store.get_person_by_id", return_value=prefs),
            patch(
                "mantle.services.secrets_service.identity_store.update_person_preferences",
                side_effect=save,
            ),
        ):
    ...
    def test_set_default_unknown_id_is_noop(self):
        ...
        with (
            patch("mantle.services.secrets_service.identity_store.get_person_by_id", return_value=prefs),
            patch("mantle.services.secrets_service.identity_store.update_person_preferences"),
        ):
