...
- :func:`porter_stem` — every canonical example from Porter (1980), grouped
  ...
  is the contract.
- :func:`tokenize` — end-to-end pipeline including
  possessive handling.
"""
...
class TestNoStopWordList:
    ...
    def test_no_stop_word_symbols_survive(self) -> None:
        ...
        assert not hasattr(_tk, "STOP_WORDS")
        assert not hasattr(_tk, "is_stop_word")

    def test_common_words_are_indexed_not_dropped(self) -> None:
...
class TestTokenize:
    def test_simple(self) -> None:
        # "quick", "brown", "fox" stem to themselves (none of the suffix rules apply).
        # "the" is kept — no stop-word stage.
    ...
    def test_common_words_and_punctuation(self) -> None:
        assert tokenize("the!  and... of?") == ["the", "and", "of"]
    ...
    def test_real_phrase(self) -> None:
        ...
        #   are → ar   ("the" and "are" are kept — no stop-word stage)
        assert result == ["the", "agenc", "ar", "manag", "artifact", "collabor"]
...
class TestBigrams:
    def test_two_stems(self) -> None:
        from mantle.search.mantle.sse.tokenizer import bigrams
