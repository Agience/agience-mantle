...
def test_rejects_external_issuer_without_bound_audience():
    ...
    v = OidcVerifier([{"issuer": iss, "jwks": {"keys": [jwk]}}])
    assert v.is_trusted(iss)
    # even a well-formed, correctly-signed token is rejected — nothing to bind aud to.
    assert v.verify(_token(priv, iss, "some-other-app", "k1")) is None
...
def test_platform_role_issuer_verifies_without_audience():
    ...
    assert not v.is_trusted(iss)
    claims = v.verify(_token(priv, iss, "anything", "k1"))
    assert claims is not None and claims["iss"] == iss
