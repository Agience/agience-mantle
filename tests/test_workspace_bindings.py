...
class TestSetBinding:
    ...
    def test_set_single_valued_binding(self):
        ...
        with (
            ...
            patch("mantle.services.workspace_service.store.get_active_grants_for_principal_resource", return_value=[MagicMock(can_read=True, can_update=True, can_create=True,
                                                   can_delete=True, can_add=True, effect="allow")]),
            patch("mantle.services.workspace_service.store.update_collection", return_value=None),
            patch("mantle.services.workspace_service.event_bus") as mock_bus,
        ):
    ...
    def test_set_multi_valued_binding(self):
        ...
        with (
            ...
            patch("mantle.services.workspace_service.store.get_active_grants_for_principal_resource", return_value=[MagicMock(can_read=True, can_update=True, can_create=True,
                                                   can_delete=True, can_add=True, effect="allow")]),
            patch("mantle.services.workspace_service.store.update_collection", return_value=None),
            patch("mantle.services.workspace_service.event_bus"),
        ):
...
class TestClearBinding:
    ...
    def test_clear_existing_binding(self):
        ...
        with (
            ...
            patch("mantle.services.workspace_service.store.get_active_grants_for_principal_resource", return_value=[MagicMock(can_read=True, can_update=True, can_create=True,
                                                   can_delete=True, can_add=True, effect="allow")]),
            patch("mantle.services.workspace_service.store.update_collection") as mock_update,
            patch("mantle.services.workspace_service.event_bus") as mock_bus,
        ):
...
class TestResolveBindingMulti:
    ...
    def test_resolve_multi_workspace_level(self):
        ...
        with (
            patch("mantle.services.workspace_service.store.get_collection_by_id", side_effect=lambda _db, cid: ws if cid == "ws-1" else _col(cid=cid)),
            patch("mantle.services.workspace_service.store.get_active_grants_for_principal_resource", return_value=[MagicMock(can_read=True, can_update=True, can_create=True,
                                                   can_delete=True, can_add=True, effect="allow")]),
        ):
    ...
    def test_resolve_multi_filters_inaccessible(self):
        ...
        def _grants(_db, grantee_id, resource_id):
            return [MagicMock(can_read=True, effect="allow")] if resource_id in {"ws-1", "c2"} else []
