...
class TestDeriveOwnerSseKey:
    def test_returns_32_bytes(self, oracle: OracleService) -> None:
        key = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        assert isinstance(key, bytes)
        assert len(key) == 32

    def test_deterministic(self, oracle: OracleService) -> None:
        # Same owner → same key, every call.
        k1 = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        k2 = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        k3 = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        assert k1 == k2 == k3

    def test_per_owner_isolation(self, oracle: OracleService) -> None:
        # Different owners → cryptographically independent keys.
        k_a = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        k_b = oracle.derive_sse_key("owner-B", self_request("owner-B", "update"))
        k_c = oracle.derive_sse_key("owner-C", self_request("owner-C", "update"))
    ...
    def test_independent_from_cell_key_tree(self, oracle: OracleService) -> None:
        ...
        sse = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        cell = oracle.derive_cell_key("owner-A", "collection-1", "anchor-1", self_request("owner-A", "update"))
        assert sse != cell
    ...
    def test_empty_principal_id_rejected(self, oracle: OracleService) -> None:
        with pytest.raises(ValueError, match="principal_id is required"):
            oracle.derive_sse_key("", self_request("someone", "update"))
    ...
    def test_survives_eviction(self, oracle: OracleService) -> None:
        ...
        before = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        oracle.evict("owner-A")
        after = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        assert before == after
...
class TestBlindToken:
    ...
    def test_owner_separation(self, oracle: OracleService) -> None:
        ...
        key_a = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        key_b = oracle.derive_sse_key("owner-B", self_request("owner-B", "update"))
