...
from mantle.search.mantle.engine import MantleHit, SystemicKeyFailure
from mantle.search.mantle.oracle import FernetMasterKeyStore
...
class TestValidation:
    def test_empty_authorized_contexts_returns_empty(self, stack):
        _, engine = stack
        assert engine.search(_vec(0), [], req()) == []

    def test_top_k_zero_returns_empty(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("a", 0, vec_seed=1)], self_request("o-1", "update"))
        result = engine.search(_vec(1), [("o-1", "col-A")], req(), top_k=0)
        assert result == []

    def test_unknown_owner_yields_empty(self, stack):
        _, engine = stack
        assert engine.search(_vec(0), [("owner-ghost", "col-X")], req()) == []

    def test_empty_query_rejected(self, stack):
        _, engine = stack
        with pytest.raises(ValueError, match="non-empty"):
            engine.search([], [("o-1", "col-A")], req())

    def test_zero_norm_query_rejected(self, stack):
        _, engine = stack
        with pytest.raises(ValueError, match="zero norm"):
            engine.search([0.0] * 16, [("o-1", "col-A")], req())

    def test_2d_query_rejected(self, stack):
        _, engine = stack
        with pytest.raises(ValueError, match="non-empty"):
            engine.search([[1.0, 2.0], [3.0, 4.0]], [("o-1", "col-A")], req())
    ...
    def test_missing_stores_rejected(self):
        ...
        with pytest.raises(ValueError, match="cell_store"):
            engine.search(_vec(0), [("o-1", "col-A")], req())
...
class TestSingleCellSearch:
    def test_finds_indexed_chunk(self, stack):
        ...
        results = engine.search(_vec(1), [("o-1", "col-A")], req())
    ...
    def test_top_k_truncates(self, stack):
        ...
        indexer.index_artifact("o-1", "col-A", chunks, self_request("o-1", "update"))
        results = engine.search(_vec(0), [("o-1", "col-A")], req(), top_k=3)
    ...
    def test_results_ordered_by_descending_score(self, stack):
        ...
        chunks = [_chunk(f"art-{i}", 0, vec_seed=i) for i in range(10)]
        indexer.index_artifact("o-1", "col-A", chunks, self_request("o-1", "update"))
        results = engine.search(_vec(0), [("o-1", "col-A")], req(), top_k=10)
    ...
    def test_hits_carry_owner_and_collection(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        [hit] = engine.search(_vec(1), [("o-1", "col-A")], req())
...
class TestMultiCollection:
    def test_dedup_picks_best_score_across_collections(self, stack):
        ...
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        indexer.index_artifact("o-1", "col-B", [_chunk("art-1", 0, vec_seed=2)], self_request("o-1", "update"))
        results = engine.search(_vec(1), [("o-1", "col-A"), ("o-1", "col-B")], req())
        assert len(results) == 1
        assert results[0].collection_id == "col-A"

    def test_results_scoped_to_authorized_contexts(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-A", 0, vec_seed=1)], self_request("o-1", "update"))
        indexer.index_artifact("o-1", "col-B", [_chunk("art-B", 0, vec_seed=1)], self_request("o-1", "update"))
        results = engine.search(_vec(1), [("o-1", "col-A")], req())
    ...
    def test_no_authorized_contexts_blocks_everything(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        assert engine.search(_vec(1), [], req()) == []
...
class TestMultiOwner:
    def test_owner_a_results_excluded_when_only_b_authorized(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-A", "col-X", [_chunk("art-1", 0, vec_seed=1)], self_request("o-A", "update"))
        indexer.index_artifact("o-B", "col-X", [_chunk("art-2", 0, vec_seed=1)], self_request("o-B", "update"))
        results = engine.search(_vec(1), [("o-B", "col-X")], req())
        ids = {h.artifact_id for h in results}
        assert ids == {"art-2"}

    def test_owner_isolation_across_search(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-A", "col-X", [_chunk("secret", 0, vec_seed=42)], self_request("o-A", "update"))
        indexer.index_artifact("o-B", "col-X", [_chunk("public", 0, vec_seed=42)], self_request("o-B", "update"))
        results = engine.search(_vec(42), [("o-B", "col-X")], req())
...
class TestCellCache:
    def test_cache_hit_avoids_decryption(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        first = engine.search(_vec(1), [("o-1", "col-A")], req())
        ...
        second = engine.search(_vec(1), [("o-1", "col-A")], req())
    ...
    def test_evict_cache_forces_reload(self, stack):
        ...
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        assert engine.search(_vec(1), [("o-1", "col-A")], req()), "precondition: search works"
        cluster = _sole_cluster(engine._cells, "o-1", "col-A")
        engine._cells.put("o-1", "col-A", b"\x00" * 64, cluster)

        # Still cached, so the corruption is not visible yet.
        assert engine.search(_vec(1), [("o-1", "col-A")], req())

        engine.evict_cache()
        with pytest.raises(SystemicKeyFailure):
            engine.search(_vec(1), [("o-1", "col-A")], req())
...
class TestResilience:
    def test_one_tampered_cell_does_not_deny_the_rest_of_the_search(self, stack):
        ...
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        indexer.index_artifact("o-1", "col-B", [_chunk("art-2", 0, vec_seed=1)], self_request("o-1", "update"))
        cluster = _sole_cluster(engine._cells, "o-1", "col-A")
        engine._cells.put("o-1", "col-A", b"\x00" * 64, cluster)
        engine.evict_cache()

        hits = engine.search(_vec(1), [("o-1", "col-A"), ("o-1", "col-B")], req())
        ids = {h.artifact_id for h in hits}
        assert "art-2" in ids, "the intact cell must still be searched"
    ...
    def test_all_cells_failing_auth_raises_instead_of_reporting_empty(self, stack):
        ...
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        indexer.index_artifact("o-1", "col-B", [_chunk("art-2", 0, vec_seed=1)], self_request("o-1", "update"))
        for coll in ("col-A", "col-B"):
            cluster = _sole_cluster(engine._cells, "o-1", coll)
            engine._cells.put("o-1", coll, b"\x00" * 64, cluster)
        engine.evict_cache()
    ...
    def test_missing_cell_does_not_crash_search(self, stack):
        ...
        _, engine = stack
        assert engine.search(_vec(0), [("o-1", "col-A")], req()) == []

    def test_chunk_with_mismatched_embedding_dim_skipped(self, stack):
        indexer, engine = stack
        indexer.index_artifact("o-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("o-1", "update"))
        from mantle.search.mantle.cell import cell_aad, pack_cell, unpack_cell
        cluster = _sole_cluster(engine._cells, "o-1", "col-A")
        aad = cell_aad("col-A", cluster)
        key = engine._oracle.derive_cell_key("o-1", "col-A", cluster, self_request("o-1", "update"))
