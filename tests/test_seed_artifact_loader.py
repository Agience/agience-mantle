...
idempotent upsert path against a mocked the lattice database.
...
def test_seed_from_artifacts_inserts_collection_then_artifact_then_edge(seeds_root, tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("mantle.services.seed_provisioning.loader.db_create_collection") as mock_create_coll,
        patch("mantle.services.seed_provisioning.loader.db_get_artifact", return_value=None),
        patch("mantle.services.seed_provisioning.loader.db_create_artifact") as mock_create_art,
        patch("mantle.services.seed_provisioning.loader.db_get_edge", return_value=None),
        patch("mantle.services.seed_provisioning.loader.db_add_artifact_to_collection") as mock_add_to_coll,
    ):
...
def test_seed_from_artifacts_is_idempotent_on_existing_artifacts(seeds_root, tmp_path, monkeypatch):
    ...
    db = MagicMock()
    with (
        patch("mantle.services.seed_provisioning.loader.db_get_collection_by_id", return_value=MagicMock()),
        patch("mantle.services.seed_provisioning.loader.db_create_collection") as mock_create_coll,
        patch("mantle.services.seed_provisioning.loader.db_get_artifact", return_value=MagicMock()),
        patch("mantle.services.seed_provisioning.loader.db_create_artifact") as mock_create_art,
...
def test_seed_from_artifacts_applies_typed_edge(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("mantle.services.seed_provisioning.loader.db_get_edge", return_value=None),
        patch("mantle.services.seed_provisioning.loader.db_add_artifact_to_collection") as mock_add,
    ):
...
def test_seed_from_artifacts_edge_idempotent_skips_existing(seeds_root, tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("mantle.services.seed_provisioning.loader.db_get_artifact", return_value=None),
        patch("mantle.services.seed_provisioning.loader.db_create_artifact"),
        patch("mantle.services.seed_provisioning.loader.db_get_edge", return_value={"_key": "exists"}),
        patch("mantle.services.seed_provisioning.loader.db_add_artifact_to_collection") as mock_add,
    ):
        report = loader.seed_from_artifacts(db, seeds_root)
...
def test_seed_from_artifacts_applies_grant(tmp_path, monkeypatch):
    ...
    db = MagicMock()
    with patch(
        "mantle.services.seed_provisioning.loader.db_upsert_user_collection_grant",
...
def test_seed_from_artifacts_grant_unresolved_principal_errors(tmp_path, monkeypatch):
    ...
    db = MagicMock()
    with patch("mantle.services.seed_provisioning.loader.db_upsert_user_collection_grant") as mock_grant:
        report = loader.seed_from_artifacts(db, root)
...
def test_seed_from_artifacts_registers_slug_in_platform_topology(seeds_root, tmp_path, monkeypatch):
    ...
    from mantle.services.platform_topology import _registry
