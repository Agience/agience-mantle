...
Foundation and subdomain are fully separate — separate stores, separate keys, separate containers
...
def test_two_separate_stores_arrive_at_the_same_digest(tmp_path: Path):
    """Foundation and subdomain, proven rather than asserted.
    ...
    subdomain, s_keys = _store(tmp_path, "subdomain")
    a = ingest_image(foundation, f_keys, image, repository="agience-mantle")
    b = ingest_image(subdomain, s_keys, image, repository="agience-mantle")
    ...
    assert read_blob(foundation, f_keys, man) == read_blob(subdomain, s_keys, man)
...
def test_a_store_cannot_read_the_other_stores_bytes(tmp_path: Path):
    ...
    image = read_layout(root)[0]
    foundation, f_keys = _store(tmp_path, "foundation")
    _subdomain, s_keys = _store(tmp_path, "subdomain")
    ...
    ingest_image(_subdomain, s_keys, image, repository="agience-mantle")
