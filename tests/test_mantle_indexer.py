...
from mantle.search.mantle import MantleIndexer
from mantle.search.mantle.cell import unpack_cell
...
class TestIndexArtifact:
    def test_chunk_round_trips_through_cell(self, indexer):
        ...
        indexer.index_artifact("owner-1", "col-A", chunks, self_request("owner-1", "update"))
        recovered = indexer.collection_chunks("owner-1", "col-A", self_request("owner-1", "update"))
    ...
    def test_multiple_artifacts_share_cell(self, indexer):
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-1", 0, vec_seed=0)], self_request("owner-1", "update"))
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-2", 0, vec_seed=1)], self_request("owner-1", "update"))
        all_chunks = indexer.collection_chunks("owner-1", "col-A", self_request("owner-1", "update"))
    ...
    def test_re_indexing_replaces_not_duplicates(self, indexer):
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-1", 0, vec_seed=1, tokens=10)], self_request("owner-1", "update"))
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-1", 0, vec_seed=1, tokens=20)], self_request("owner-1", "update"))
        all_chunks = indexer.collection_chunks("owner-1", "col-A", self_request("owner-1", "update"))
        assert len(all_chunks) == 1
        assert all_chunks[0]["tokens"] == 20

    def test_different_collections_get_separate_cells(self, indexer):
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-1", 0, vec_seed=5)], self_request("owner-1", "update"))
        indexer.index_artifact("owner-1", "col-B", [_chunk("art-2", 0, vec_seed=5)], self_request("owner-1", "update"))
        cells = indexer.cells_for("owner-1")
        assert set(cells) == {"col-A", "col-B"}

    def test_empty_chunk_list_is_no_op(self, indexer):
        result = indexer.index_artifact("owner-1", "col-A", [], self_request("owner-1", "update"))
        assert result == 0
    ...
    def test_validation_rejects_non_dict(self, indexer):
        with pytest.raises(ValueError, match="dict"):
            indexer.index_artifact("owner-1", "col-A", ["not a dict"], self_request("owner-1", "update"))

    def test_rejects_empty_owner_or_collection(self, indexer):
        with pytest.raises(ValueError):
            indexer.index_artifact("", "col-A", [_chunk("a", 0)], self_request(""))
        with pytest.raises(ValueError):
            indexer.index_artifact("owner-1", "", [_chunk("a", 0)], self_request("owner-1", "update"))

    def test_returns_1_when_cell_touched(self, indexer):
        result = indexer.index_artifact("owner-1", "col-A", [_chunk("art-1", 0)], self_request("owner-1", "update"))
        assert result == 1
...
class TestRemoveArtifact:
    def test_removes_artifact_chunks(self, indexer):
        ...
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-2", 0, vec_seed=2)], self_request("owner-1", "update"))
        touched = indexer.remove_artifact("owner-1", "col-A", "art-1", self_request("owner-1", "update"))
        assert touched == 1
        remaining = {c["artifact_id"] for c in indexer.collection_chunks("owner-1", "col-A", self_request("owner-1", "update"))}
        assert remaining == {"art-2"}

    def test_removing_only_artifact_deletes_cell(self, indexer):
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-only", 0, vec_seed=1)], self_request("owner-1", "update"))
        indexer.remove_artifact("owner-1", "col-A", "art-only", self_request("owner-1", "update"))
        assert indexer.cells_for("owner-1") == []

    def test_remove_unknown_artifact_is_no_op(self, indexer):
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("owner-1", "update"))
        touched = indexer.remove_artifact("owner-1", "col-A", "ghost", self_request("owner-1", "update"))
        assert touched == 0
        assert len(indexer.collection_chunks("owner-1", "col-A", self_request("owner-1", "update"))) == 1

    def test_remove_only_touches_target_collection(self, indexer):
        indexer.index_artifact("owner-1", "col-A", [_chunk("art-1", 0, vec_seed=1)], self_request("owner-1", "update"))
        indexer.index_artifact("owner-1", "col-B", [_chunk("art-1", 0, vec_seed=1)], self_request("owner-1", "update"))
        indexer.remove_artifact("owner-1", "col-A", "art-1", self_request("owner-1", "update"))
    ...
    def test_remove_validation(self, indexer):
        with pytest.raises(ValueError):
            indexer.remove_artifact("", "col-A", "art-1", self_request(""))
        with pytest.raises(ValueError):
            indexer.remove_artifact("owner-1", "", "art-1", self_request("owner-1", "update"))
        with pytest.raises(ValueError):
            indexer.remove_artifact("owner-1", "col-A", "", self_request("owner-1", "update"))
...
class TestMultiOwnerIsolation:
    def test_two_owners_have_independent_storage(self, indexer):
        indexer.index_artifact("owner-A", "col-X", [_chunk("art-1", 0, vec_seed=1)], self_request("owner-A", "update"))
        indexer.index_artifact("owner-B", "col-X", [_chunk("art-1", 0, vec_seed=1)], self_request("owner-B", "update"))
