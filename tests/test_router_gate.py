...
`chorus.private.pem`; gate_router verifies via `origin.authority_trust`. These
...
from mantle.services.server_registry import all_client_ids as _all_client_ids


def _platform_persona_payload(client_id: str | None = None) -> dict:
    """Shape of a verified Chorus platform JWT payload (Phase C)."""
    # Resolve at call time (the server registry is populated per-test by the
    # conftest fixture, not at import) — fall back to a synthetic id if empty.
    cid = client_id or (sorted(_all_client_ids()) or ["agience-server-aria"])[0]
    return {
        "iss": "chorus",
        "sub": cid,
        "aud": "mantle",
...
def _patch_verify_ok(payload):
    ...
    return patch("mantle.services.oidc.get_oidc_verifier", return_value=verifier)
...
class TestGetUsage:
    ...
    async def test_returns_limits_and_usage_shape(self, client: AsyncClient):
        with (
            ...
            patch(
                "mantle.services.gate_service.get_or_default_limits",
                return_value={"max_workspaces": 10, "max_artifacts": 100, "vu_limit": 1000},
            ),
            patch("mantle.services.gate_service.count_workspaces", return_value=3),
            patch("mantle.services.gate_service.count_artifacts", return_value=42),
            patch("mantle.services.gate_service.get_tally", return_value=125),
        ):
            r = await client.get(
                "/internal/gate/usage/user-1", headers=_bearer()
            )
...
class TestSetLimits:
    ...
    async def test_delegates_to_gate_service(self, client: AsyncClient):
        with (
            _patch_verify_ok(_platform_persona_payload()),
            patch("mantle.services.gate_service.set_limits") as set_limits,
        ):
            r = await client.post(
                "/internal/gate/set-limits",
                json={
