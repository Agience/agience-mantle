...
from mantle.search.mantle.sse.unified import UnifiedHit

_RESOLVE = "mantle.search.mantle.sse.router_accessor.resolve_authorized_contexts"
...
class _FakeUnified:
    ...
    def search(self, query_text, contexts, request=None, *, query_embedding, top_k, **kw):
...
def _accessor(hits):
    return MantleSseSearchAccessor(
        _FakeUnified(hits), object(), store_db=object(), embeddings=_FakeEmbeddings(),
    )
