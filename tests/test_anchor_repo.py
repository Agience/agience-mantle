...
Covers the in-memory repo (geometry tests stay db-free) and the lattice repo's
...
from mantle.search.anchors.anchorset import Anchor, AnchorSet, l2norm
from mantle.search.anchors.repo import StoreAnchorRepo, InMemoryAnchorRepo
...
# ---------------------------------------------------------------------------
# the lattice repo against a minimal fake DB
# ---------------------------------------------------------------------------
...
class _FakeDB:
    """Just enough of the lattice handle for StoreAnchorRepo via the db.store
...
def test_lattice_repo_add_and_load(monkeypatch):
    ...
    monkeypatch.setattr(
        db_store, "get_collection_by_id",
        lambda _db, cid: object() if cid == "anchorset-col" else None,
    )

    monkeypatch.setattr(db_store, "create_artifact", fake_create_artifact)
    monkeypatch.setattr(db_store, "get_artifact", fake_get_artifact)
    monkeypatch.setattr(db_store, "add_artifact_to_collection", fake_add_to_collection)
    monkeypatch.setattr(db_store, "list_collection_artifacts", fake_list_collection_artifacts)

    repo = StoreAnchorRepo(db)
    src = _aset()
    repo.bulk_add(src.anchors)
...
def test_lattice_repo_self_provisions_collection(monkeypatch):
    ...
    from mantle.db import backend as db_store
    from mantle.services import platform_topology, peer_signing
    ...
    monkeypatch.setattr(db_store, "get_collection_by_id", lambda _db, cid: None)
    monkeypatch.setattr(db_store, "create_collection",
    ...
    monkeypatch.setattr(db_store, "create_artifact", fake_create_artifact)
    monkeypatch.setattr(db_store, "get_artifact", lambda _db, aid: db.docs.get(aid))
    monkeypatch.setattr(db_store, "add_artifact_to_collection", lambda *a, **k: True)

    StoreAnchorRepo(db).bulk_add(_aset().anchors)
