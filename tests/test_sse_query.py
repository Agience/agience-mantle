...
def oracle() -> OracleService:
    ...
    return make_oracle(FernetMasterKeyStore(fernet))
...
def _seed_corpus(indexer: SseIndexer, owner: str = "owner-A") -> None:
    ...
    """
    indexer.index_artifact(owner, "col-1", "art-1", {"title": "encryption library"}, self_request(owner, "update"))
    indexer.index_artifact(owner, "col-1", "art-2", {"title": "library cards"}, self_request(owner, "update"))
    indexer.index_artifact(owner, "col-2", "art-3", {"title": "encryption keys"}, self_request(owner, "update"))
...
class TestShortCircuits:
    def test_empty_query(self, engine):
        assert engine.search("", [("owner-A", "col-1")], req()) == []
        assert engine.search("   ", [("owner-A", "col-1")], req()) == []

    def test_top_k_zero_or_negative(self, engine, indexer):
        _seed_corpus(indexer)
        assert engine.search("encryption", [("owner-A", "col-1")], req(), top_k=0) == []
        assert engine.search("encryption", [("owner-A", "col-1")], req(), top_k=-1) == []

    def test_no_authorized_contexts(self, engine, indexer):
        _seed_corpus(indexer)
        assert engine.search("encryption", [], req()) == []
        # Skipping owner-less / collection-less tuples.
        assert engine.search("encryption", [("", "col-1")], req()) == []
        assert engine.search("encryption", [("owner-A", "")], req()) == []
...
class TestBasicSearch:
    def test_finds_indexed_term(self, engine, indexer):
        _seed_corpus(indexer)
        hits = engine.search(
            "encryption", [("owner-A", "col-1"), ("owner-A", "col-2")], req())
    ...
    def test_returns_sse_hit_shape(self, engine, indexer):
        _seed_corpus(indexer)
        hits = engine.search("encryption", [("owner-A", "col-1")], req())
        assert len(hits) >= 1
        for hit in hits:
    ...
    def test_unmatched_query_returns_empty(self, engine, indexer):
        _seed_corpus(indexer)
        hits = engine.search(
            "zebra", [("owner-A", "col-1"), ("owner-A", "col-2")], req())
        assert hits == []
    ...
    def test_query_stem_matches_index_stem(self, engine, indexer):
        ...
        hits = engine.search("runs", [("owner-A", "col-1")], req())
        assert len(hits) == 1
    ...
    def test_multi_term_query(self, engine, indexer):
        _seed_corpus(indexer)
        hits = engine.search(
            "encryption library", [("owner-A", "col-1")], req())
...
class TestAuthorizationFilter:
    ...
    def test_filters_in_shared_posting_list(self, engine, indexer):
        ...
        # "library".
        hits = engine.search("library", [("owner-A", "col-2")], req())
        assert hits == []
...
class TestMultiOwner:
    def test_search_across_owners(self, engine, indexer):
        ...
        self_request("owner-B", "update"))
        hits = engine.search(
            "encryption",
            [("owner-A", "col-1"), ("owner-B", "col-X")], req())
...
class TestRanking:
    def test_top_k_truncation(self, engine, indexer):
        ...
        hits = engine.search(
            "encryption",
            [("owner-A", "col-1"), ("owner-A", "col-2")], req(),
            top_k=1)
        assert len(hits) == 1
...
class TestFieldSelection:
    def test_explicit_fields_param(self, oracle, posting_store, stats_store, indexer):
        ...
        title_hits = eng.search(
            "lazy", [("owner-A", "col-1")], req(), fields=["title"])
        # Search content only.
        content_hits = eng.search(
            "lazy", [("owner-A", "col-1")], req(), fields=["content"])
    ...
    def test_zero_boost_field_skipped(
        ...
        hits = eng.search("alpha", [("owner-A", "col-1")], req())
    ...
    def test_field_boost_scales_score(
        ...
        default_score = eng_default.search("alpha", [("owner-A", "col-1")], req())[0].score
        boosted_score = eng_boosted.search("alpha", [("owner-A", "col-1")], req())[0].score
    ...
    def test_unknown_field_in_explicit_list_ignored(
        ...
        hits = eng.search(
            "alpha", [("owner-A", "col-1")], req(), fields=["title", "garbage"])
        assert len(hits) == 1
...
class TestCaching:
    def test_posting_cache_warm_skips_store(
        ...
        eng = SseQueryEngine(oracle, posting_store, stats_store)
        # Warm cache.
        eng.search("alpha", [("owner-A", "col-1")], req())
    ...
    def test_invalidate_caches_drops_owner(
        ...
        self_request("owner-A", "update"))
        eng = SseQueryEngine(oracle, posting_store, stats_store)
        eng.search("alpha", [("owner-A", "col-1")], req())  # warm cache
        ...
        # missing data.
        assert eng.search("alpha", [("owner-A", "col-1")], req()) == []
    ...
    def test_invalidate_caches_global(
        ...
        self_request("owner-A", "update"))
        eng = SseQueryEngine(oracle, posting_store, stats_store)
        eng.search("alpha", [("owner-A", "col-1")], req())  # warm
...
class TestResilience:
    def test_tampered_posting_silently_skipped(
        ...
        owner_key = oracle.derive_sse_key("owner-A", self_request("owner-A", "update"))
        ...
        beta_hits = eng.search("beta", [("owner-A", "col-1")], req())
...
class TestPhraseSearch:
    ...
    def test_phrase_stops_word_stripped(self, engine, indexer):
        ...
        self_request("owner-A", "update"))
        hits = engine.search('"the library"', [("owner-A", "col-1")], req())
        assert {h.artifact_id for h in hits} == {"art-1"}
    ...
    def test_phrase_empty_after_strip_returns_empty(self, engine, indexer):
        ...
        indexer.index_artifact(
            "owner-A", "col-1", "art-1", {"title": "encryption library"},
        self_request("owner-A", "update"))
        hits = engine.search('"the and"', [("owner-A", "col-1")], req())
        assert hits == []
