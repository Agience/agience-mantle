"""`search/mantle/key_provider.py` — KEK custody selection.

This module wraps the platform KEK that protects every per-principal master DEK
(`oracle.LatticeMasterKeyStore`).
...
silently descended the maturity ladder the module exists to climb. Every test below asserts that
misconfiguration raises rather than degrades; a round-trip-only suite would pass against a version
that defaulted to local on error.
"""
...
def test_an_unknown_provider_raises_instead_of_falling_back(monkeypatch, value):
    """The core guarantee: a silent fallback to `local` would mean a managed deployment kept
...
def test_the_selector_tolerates_case_and_whitespace(monkeypatch):
    ...
    the operator meant, rather than raise. Only genuinely unknown values raise."""
...
def test_kms_token_is_base64_not_raw_bytes():
    ...
    `base64.b64decode` defaults to `validate=False`, which silently discards every character
    outside the base64 alphabet and decodes whatever is left rather than raising — so decoding
    alone does not verify well-formedness. `validate=True` is the keyword that makes the check a
    check.
    """
    p = AwsKmsKeyProvider("k", client=_FakeKms())
    token = p.wrap(DEK)
    assert isinstance(token, str), "the token must be TEXT — raw bytes corrupt on the JSON boundary"
    base64.b64decode(token, validate=True)          # raises on anything outside the alphabet
