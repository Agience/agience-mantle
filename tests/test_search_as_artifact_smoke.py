...
encrypted-search accessor (tests never touch a real Oracle/S3/the lattice).
"""
...
from mantle.services import operation_dispatcher
from mantle.services.operation_dispatcher import DispatchContext
...
def _builtin_handlers():
    ...
    from mantle.services.handler_registry import register_builtin_handlers
...
def _ctx(artifact_id="search-std"):
    ...
    return DispatchContext(user_id="u1", actor_id="u1", grants=[grant], store_db=MagicMock())
...
async def test_standard_flavor_routes_through_dispatcher_to_run_search():
    ...
    with patch("mantle.search.mantle.wiring.build_sse_search_accessor", return_value=_fake_accessor_with_hits()):
...
async def test_premium_flavor_denied_without_entitlement():
    ...
    with patch("mantle.services.gate_service.has_feature", return_value=False):
...
async def test_premium_flavor_entitled_but_server_unregistered_degrades_503():
    ...
    with patch("mantle.services.gate_service.has_feature", return_value=True):
        with pytest.raises(HTTPException) as ei:
            await operation_dispatcher.dispatch("invoke", art, {"query_text": "x"}, _ctx())
