...
#: empty and every recall answers `ordering: "coverage"`, the correct answer for a node with no
#: ontology in it. An editor talking to that node over MCP gets the reach-ranked `recall` the
#: moment the ontology is in the same process.
#:
...
def _registered_seam(name: str):
    ...
    `bind()` requires someone to call it; a runner answers the same question without that call.
    `prism.runner.register_seam(name, dotted_module)` is how a host says which module fills a
    name, and `ember/runtime/seams.py` registers `match` and `projection` at import of `ember`.
    Reading that map here is what makes mantle + a runner enough.
    ...
    static import graph names no ember or crystal module.
...
def _resolve_seam(name: str):
    """The seam, proven reachable rather than merely present.
    ...
    later where the failure would escape as a traceback instead of the "unavailable" this module
    reports.
    """
...
def _salient_title(doc: dict, store, matcher) -> str:
    ...
    The measurement that keeps it out of the path: it prices at +2 on `bench_canon --by-title`
    while every OEWN distance was 0 (§123), and at -3 once `refresh_ic` restored the metric.
    Trimming a title's generic anchors helps only when the anchors cannot be told apart by
    distance; when they can, the generic ones are already far and dropping them removes real reach.
    ...
    information. A title is a piece of text. Applying the same measure to it is the same rule on
    the same seam: `README`/`Read` survive, `in`/`this`/`order` do not, and a title that is all
    subject — `prism-protocol` — is untouched.
    ...
    from the seam per call, so a reference to it by name here would raise `NameError` into this
    function's own `except` and hand back every title unchanged in every process.
    """
...
def _reach_rank(cand: List[Tuple[str, str, float]], query: str, store,
    ...
    try:
        ...
        # and the answer is `{"reach": "unavailable"}`.
    ...
    def _is_modifier(name: str) -> bool:
        ...
        as this can tell, and falls through to being its own position.
        """
    ...
    def _position(cid: str) -> List[str]:
        ...
        admitted directly would score a distance that came from nowhere. A modifier answer read at
        its own name is placed where no question can reach it. Measured:
        ...
        the position they had, which is the correct outcome — the source holds no link for them.
        """
    ...
    #
    # The default is `True`. For a corpus search the injection is right and load-bearing — the
    ...
    # on its own, so the per-position reads happen either way, and a batch call over the same
    # targets recomputes exactly the distances they just computed. Positions repeat heavily across
    # candidates — every article about glaciers stands on `glacier.n.01` — so the per-position read
    # is cached across the whole pool, which a batch call cannot be.
    #
    # A position whose synset does not resolve reads `(0.0, inf)`, the same value a batch call
    # gives it by skipping it, so the identity holds there too.
    ...
    # 200 read one row at a time is 200 round trips — and the pass is I/O bound, not compute bound:
    # sampled during a live run it spent 46 seconds of CPU in 300 seconds of wall clock.
    ...
    #
    # A missing id simply does not come back.
    #
        ...
        if not targets and cid in _docs:         # prose: the terms the document itself is keyed on
            try:
                ...
                if row:
                    ...
                    #
                    # A doc row carries no body to read instead: measured on the live corpus, a
                    # wiki artifact's doc row holds 6 to 50 characters of text (mean 12), because
                    # `content` lives in CAS behind decryption and never reaches this read. Read
                    # from the row's own text, the position would come from the title alone.
                    #
                    ...
                    # `lemmas` beats a title for wiki prose, and lemmas alone lose the subject of
                    # a sectioned document: `astra/doc_index` extracts a section's key terms from
                    ...
                    #                  (109 terms, none of them agent, host, or design)
                    #
                    # Without the title, the query "agent host design" places that section away
                    # from its own subject and `UNIVERSAL-ECONOMICS#intro` — which never says it
                    # in its title but does in its body — outranks it. The title is the only place
                    # a section carries what document it belongs to.
                    #
                    ...
                    #
                    # `getattr` because a seam may carry only the offer half; its absence is the
                    # noun-only answer.
                    lem = d.get("lemmas") or []
                    ...
                    #
                    # Reached through `getattr` because a seam may carry no such function —
                    # an absent core is "nothing to group", which is the full position set.
    ...
    #
    # `energy / n` makes the answers lexicon-only: a concept always has exactly one position, the
    # smallest denominator available, so dividing by count hands it a structural win no measurement
    # gave it.
    #
...
def _fold_key(doc: Dict[str, Any]):
    ...
    **The Princeton side carries one on 90.6% of its rows.** Measured 2026-08-26 over the `wn-` id
    range on 71/home: pwn 106,640 of 117,659 (90.6%), oewn 120,630 of 120,630, omw 437,930 of
    437,936. The mapping is derived from this corpus — OMW ids encode the PWN 3.0 offset and OMW
    rows carry the ILI, so `Princeton synset → offset → wn-omw-*-<offset>-<pos> → ili` is a local
    derivation. Nothing is downloaded: an external map would be a claim about this corpus from
    outside it, needing its own provenance, where this is the corpus agreeing with itself. Across
    117,187 OMW (offset, pos) keys the languages disagree on the ILI **0 times**.

    ILI is a first key rather than a replacement, because of the 11,019 rows (9.4%) that carry
    none: 10,866 have no OMW twin at their offset, and 153 are refusals where OEWN publishes a
    different ILI — counted, not guessed. Those fall through to the lemma key below, which pairs
    them with OEWN as measured.
...
def _fold_sources(ranked: List[Tuple[str, str, float]], docs: Dict[str, str]
    ...
    lexicons carry is the key, and the Princeton side carries one too — see `_fold_key`. It is a
    first key and not a replacement, because 9.4% of Princeton rows have none and fall through to
    the lemma key described here.
    ...
    because they are where a caller finds the other source's gloss.
    """
    import json as _json
...
def _cut_for(ranked: Sequence[Tuple[str, str, float]], *, query: str | None = None,
    ...
    adaptive instrument has no coordinates to read and declines, so the cut falls back to `_knee`
    and a caller that omits the argument gets the thin tier. Measured on the live corpus without a
    frame, four of six queries cut to 50 of 50, which is no cut at all. The frame is derivable from
    the ranked list and the store, so deriving it here is what stops the argument from being
    forgettable.
    ...
    cut is `_knee` over the scores, which is the thin-tier answer.

    `frame=` wins when a caller already holds one: building it twice would read the same
...
# ── the published names ──────────────────────────────────────────────────────────────────────────
# The underscored names are the implementation; these are the published spellings a caller reaches
# for, so a caller such as `sage.content_search` can keep its own private names pointing here.
knee = _knee
