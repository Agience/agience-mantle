...
from services.dependencies import resolve_auth, require_platform_admin, AuthContext
from agience_core.config import AUTHORITY_ISSUER
...
def test_resolve_auth_accepts_direct_api_key():
    ...
    # OriginClient.verify_api_key returns (APIKey, [Grant, ...]) tuple.
    fake_origin.verify_api_key.return_value = (api_key, mock_grants)

    with patch("services.dependencies.get_origin_client", return_value=fake_origin):
...
def test_resolve_auth_parses_artifact_prefix():
    ...
    fake_origin = MagicMock()
    fake_origin.verify_api_key.return_value = (api_key, [])
...
def test_resolve_auth_accepts_grant_key_in_bearer():
    ...
    CRUDEASIO lives in Mantle — grant-key lookup goes to Arango, not Origin.
    """
    grant = GrantEntity(
        id="grant-1",
        resource_id="col-1",
    ...
    with patch("services.dependencies.verify_token", return_value=None), \
         patch("services.dependencies.db_arango.get_active_grants_by_key", return_value=[grant]):
...
def test_resolve_auth_invalid_token_raises_401():
    ...
    with patch("services.dependencies.verify_token", return_value=None), \
         patch("services.dependencies.db_arango.get_active_grants_by_key", return_value=[]):
        with pytest.raises(HTTPException) as exc_info:
...
def test_operator_fastpath_open_before_any_authority_grant():
    ...
    with (
        patch("services.platform_settings_service.settings.get", return_value="op-1"),
        patch("services.dependencies.get_id", return_value="auth-col"),
        patch("db.arango.get_grants_for_collection", return_value=[]),
    ):
...
def test_operator_fastpath_closes_after_authority_admin_grant():
    ...
    with (
        patch("services.platform_settings_service.settings.get", return_value="op-1"),
        patch("services.dependencies.get_id", return_value="auth-col"),
        patch("db.arango.get_grants_for_collection", return_value=[_admin_grant()]),
        patch("db.arango.get_active_grants_for_principal_resource", return_value=[]),
    ):
...
def test_operator_with_own_grant_passes_after_window_closed():
    ...
    with (
        ...
        patch("services.dependencies.get_id", return_value="auth-col"),
        patch("db.arango.get_grants_for_collection", return_value=[_admin_grant()]),
        patch("db.arango.get_active_grants_for_principal_resource", return_value=[_admin_grant()]),
    ):
        assert require_platform_admin(_operator_auth(), MagicMock()) == "op-1"
