...
class TestAuthGuards:
    ...
    async def test_invoke_requires_user(self, anon_client: AsyncClient):
        r = await anon_client.post("/artifacts/a-1/op/invoke", json={})
        assert r.status_code == 401
...
class TestCreateArtifact:
    ...
    async def test_create_top_level_artifact(self, client: AsyncClient):
        ...
        with patch("services.workspace_service.create_container", return_value=ws) as mk:
            r = await client.post(
                "/artifacts",
    ...
    async def test_create_top_level_collection(self, client: AsyncClient):
        ...
        with patch("services.workspace_service.create_container", return_value=col):
            r = await client.post(
                "/artifacts",
...
class TestRemoveArtifactFromWorkspace:
    ...
    async def test_happy_path_returns_removed_response(self, client: AsyncClient):
        ...
        with patch(
            "services.workspace_service.remove_artifact_from_container",
    ...
    async def test_undeclared_invoke_returns_404(self, client: AsyncClient):
        ...
        try:
            with patch(
                ...
                r = await client.post("/artifacts/art-1/op/invoke", json={})
        finally:
            app.dependency_overrides.pop(get_arango_db, None)
        assert r.status_code == 404
...
class TestReorder:
    ...
    async def test_works_on_any_container(self, client: AsyncClient):
        ...
        try:
            with patch(
                "db.arango.get_artifact",
                side_effect=lambda _db, aid: SimpleNamespace(root_id=aid),
            ), patch(
                "db.arango.reorder_collection_artifacts",
                return_value=None,
            ) as svc:
                r = await client.patch(
                    "/artifacts/container-1/children/order",
...
class TestMoveArtifact:
    ...
    async def test_happy_path_passes_collection_id_as_source(
        ...
        try:
            with patch(
                "services.workspace_service.move_artifact_between_containers",
                return_value=moved,
            ) as svc:
                r = await client.post(
                    "/artifacts/art-1/op/move",
                    json={"target_container_id": "ws-2"},
                )
        finally:
            app.dependency_overrides.pop(get_arango_db, None)
        ...
        assert svc.call_args.kwargs["source_container_id"] == "container-1"
        assert svc.call_args.kwargs["target_container_id"] == "ws-2"
...
class TestSearchArtifacts:
    ...
    async def test_happy_path_returns_hits(self, client: AsyncClient):
        ...
        OpenSearch retirement (Step 2.6.9). Patch the wiring builder to
        ...
        with patch(
            "search.mantle.wiring.build_sse_search_accessor",
    ...
    @pytest.mark.asyncio
    async def test_embedding_mode_passes_raw_vector_to_accessor(self, client: AsyncClient):
        """"Embedding activation": posting `embedding` (no query_text) threads the
        raw vector to the accessor as `query.query_embedding`, with empty text."""
        captured = {}
    ...
    @pytest.mark.asyncio
    async def test_400_when_both_query_text_and_embedding(self, client: AsyncClient):
        r = await client.post(
            "/artifacts/search",
            json={"query_text": "x", "embedding": [0.1, 0.2]},
        )
        assert r.status_code == 400
...
class TestEmbeddingEndpoint:
    @pytest.mark.asyncio
    async def test_get_embedding_returns_stored_vector(self, client: AsyncClient):
        doc = {"id": "a-1", "root_id": "a-1", "collection_id": "ws-1",
               "state": "committed", "context": "{}"}
        with (
            patch("routers.artifacts_router.check_access"),
            patch("routers.artifacts_router._find_artifact", return_value=doc),
            patch("search.ingest.pipeline_unified.get_artifact_embeddings",
                  return_value=[{"chunk_id": 0, "embedding": [0.1, 0.2], "model_id": "m"}]),
        ):
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_404_when_none(self, client: AsyncClient):
        doc = {"id": "a-1", "root_id": "a-1", "collection_id": "ws-1",
               "state": "committed", "context": "{}"}
        with (
            patch("routers.artifacts_router.check_access"),
            patch("routers.artifacts_router._find_artifact", return_value=doc),
            patch("search.ingest.pipeline_unified.get_artifact_embeddings", return_value=[]),
        ):
            r = await client.get("/artifacts/a-1/embedding")
        assert r.status_code == 404

    @pytest.mark.asyncio
    async def test_activate_echoes_carrier_vector(self, client: AsyncClient):
        with (
            patch("search.anchors.activate.activate_vector", return_value={"anchors": []}),
            patch("search.mantle.wiring.build_sse_search_accessor", return_value=None),
        ):
