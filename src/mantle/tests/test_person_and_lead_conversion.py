...
import services.seed_provisioning.user_provisioning as up  # noqa: E402
from services.seed_provisioning.loader import UserContext  # noqa: E402
...
def test_ensure_people_collection_creates_and_grants_operator(monkeypatch):
    ...
    monkeypatch.setattr("services.platform_settings_service.settings.get",
...
def test_ensure_people_collection_idempotent(monkeypatch):
    ...
    monkeypatch.setattr(up, "db_create_collection", lambda db, c: created.append(c))
    monkeypatch.setattr(up, "_persist_seed_ids", lambda db, m: None)
    monkeypatch.setattr("services.platform_settings_service.settings.get", lambda key, *a, **k: None)
...
def test_send_welcome_email_sends_as_operator(monkeypatch):
    ...
    monkeypatch.setattr("services.platform_settings_service.settings.get",
                        lambda key, *a, **k: "op-1" if key == "platform.operator_id" else None)
    monkeypatch.setattr("services.server_registry.resolve_name_to_id", lambda name: "iris-uuid")
    monkeypatch.setattr("services.chorus_client.call_tool",
...
def test_send_welcome_email_noop_without_email(monkeypatch):
    calls: list = []
    monkeypatch.setattr("services.chorus_client.call_tool", lambda *a, **k: calls.append(1))
