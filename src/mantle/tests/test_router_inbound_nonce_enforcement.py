...
from services.dependencies import AuthContext, get_auth, get_arango_db  # noqa: E402
...
class TestInvokeOpNonce:
    def test_invoke_without_challenge_is_403(self, inbound_client):
        resp = inbound_client.post(
    ...
    def test_invoke_with_bad_challenge_is_403(self, inbound_client):
        resp = inbound_client.post(
            f"/artifacts/{_ARTIFACT_ID}/op/invoke",
            json={"params": {"messages": "[]"}},
            headers={"X-Agience-Challenge": "garbage"},
        )
        assert resp.status_code == 403
        assert "Invalid or expired nonce" in resp.json()["detail"]
