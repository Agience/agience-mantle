...
from services import mcp_resource_importer as imp
...
class TestCapabilityIndexing:
    def test_create_indexes_into_committed(self):
        with (
            patch("services.mcp_resource_importer.db_get_artifact", return_value=None),
            patch("services.mcp_resource_importer.db_create_artifact"),
            patch("services.mcp_resource_importer.db_add_artifact_to_collection"),
    ...
    def test_recommit_from_archived_vacates_archived(self):
        ...
        with (
            patch("services.mcp_resource_importer.db_get_artifact", return_value=existing),
            patch("services.mcp_resource_importer.db_update_artifact"),
            patch("search.ingest.pipeline_unified.enqueue_index_artifact") as idx,
        ):
            imp._upsert_capability_artifact(
                MagicMock(), artifact_id="cap-1", collection_id="col-1",
    ...
    def test_archive_stale_vacates_committed(self):
        ...
        with (
            patch("services.mcp_resource_importer._db_list_collection_artifacts",
                  return_value=[stale]),
            patch("services.mcp_resource_importer.db_update_artifact"),
            patch("search.ingest.pipeline_unified.enqueue_index_artifact") as idx,
        ):
