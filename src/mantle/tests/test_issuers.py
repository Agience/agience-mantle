...
from services.issuers import ISSUER_CONTENT_TYPE, load_issuer_configs
...
class TestLoadIssuerConfigs:
    def test_fail_closed_when_system_principal_unknown(self):
        ...
        with (
            patch("services.peer_signing.get_system_principal_id", return_value=None),
            patch("db.arango.list_committed_artifacts_by_context_content_type") as q,
        ):
    ...
    def test_maps_system_owned_committed_artifacts(self):
        with (
            patch("services.peer_signing.get_system_principal_id", return_value="SYS"),
            patch("db.arango.list_committed_artifacts_by_context_content_type",
...
class TestVerifierRefresh:
    def test_refresh_merges_external_issuer(self):
        ...
        with patch("services.issuers.load_issuer_configs",
    ...
    def test_handle_issuer_event_refreshes(self):
        ...
        fake_db = object()
        with patch("services.oidc.get_oidc_verifier") as gv:
    ...
    async def test_watch_refreshes_on_published_issuer_event(self):
        ...
        refreshed = []
        with (
            patch("services.dependencies.get_arango_db",
                  side_effect=lambda: iter([object()])),
            patch("services.oidc.get_oidc_verifier") as gv,
        ):
...
class TestAdminCreateRevoke:
    def test_create_issuer_artifact_is_system_owned(self):
        ...
        with (
            patch("services.peer_signing.get_system_principal_id", return_value="SYS"),
            patch("db.arango.create_artifact",
    ...
    def test_create_requires_keys(self):
        from services import issuers
        with patch("services.peer_signing.get_system_principal_id", return_value="SYS"):
            with pytest.raises(ValueError):
                issuers.create_issuer_artifact(object(), issuer="x", authorized_by="a")

    def test_create_fails_without_system_principal(self):
        from services import issuers
        with patch("services.peer_signing.get_system_principal_id", return_value=None):
    ...
    def test_revoke_archives_issuer(self):
        ...
        updated = {}
        with (
            patch("db.arango.get_artifact", return_value=art),
            patch("db.arango.update_artifact",
    ...
    def test_revoke_rejects_non_issuer(self):
        ...
        with patch("db.arango.get_artifact", return_value=art):
...
class TestIssuerRouter:
    async def test_create_requires_admin(self, client):
        with patch("routers.issuers_router.require_platform_admin",
    ...
    async def test_create_dispatches_to_service_as_admin(self, client):
        with (
            patch("routers.issuers_router.require_platform_admin", return_value="admin-1"),
            patch("services.issuers.create_issuer_artifact",
...
class TestSeedPlatformIssuers:
    def test_seeds_manifest_anchors_and_env(self):
        ...
        with (
            patch("services.peer_signing.get_system_principal_id", return_value="SYS"),
            patch("services.issuers.list_issuer_artifacts", return_value=[]),
            patch("services.oidc._read_authority_manifest", return_value=manifest),
            patch("agience_core.config.AUTHORITY_ISSUER", "https://auth.test"),
            patch("agience_core.config.TRUSTED_ISSUERS",
    ...
    def test_seed_is_idempotent(self):
        ...
        with (
            patch("services.peer_signing.get_system_principal_id", return_value="SYS"),
            patch("services.issuers.list_issuer_artifacts", return_value=[existing]),
            patch("services.oidc._read_authority_manifest", return_value=manifest),
            patch("agience_core.config.AUTHORITY_ISSUER", None),
            patch("agience_core.config.TRUSTED_ISSUERS", [], create=True),
            patch("services.issuers.create_issuer_artifact",
                  side_effect=lambda db, **kw: created.append(kw)),
        ):
            n = issuers.seed_platform_issuer_artifacts(object())
...
class TestContainerEmitsChangeEvent:
    def test_create_collection_emits_artifact_created(self):
        ...
        from db import arango as dba
        entity = MagicMock()
        with (
            patch("db.arango.insert_document", return_value=entity),
            patch("db.arango._emit_artifact_change") as emit,
        ):
            dba.create_collection(MagicMock(), entity)
...
class TestVerifierRefreshUnknownIss:
    def test_refresh_if_unknown_iss_throttled(self):
        v = OidcVerifier(trusted_issuers=[])
        with (
            patch("services.oidc.jwt.get_unverified_claims",
                  return_value={"iss": "https://new.test"}),
            patch("services.issuers.load_issuer_configs",
            ...
            with patch("services.oidc.jwt.get_unverified_claims",
