...
def create_collection(db: StandardDatabase, entity: CollectionEntity) -> CollectionEntity:
    # A container IS an artifact — emit the same chokepoint change event as
    # create_artifact so container creates also drive the change-feed + watchers
    # ("everything is an artifact, and deserves an event").
    result = insert_document(db, entity, COLLECTION_ARTIFACTS)
    _emit_artifact_change(entity, "artifact.created")
    return result
...
def _emit_artifact_change(entity, event_name: str) -> None:
    """Emit a change event for an artifact write.

    Every artifact write goes through create_artifact / update_artifact, so emitting
    HERE makes the change-feed complete by construction — "everything is an artifact,
    and deserves an event". No write is silent, including raw ones (the Person card,
    provisioning, seeds). Best-effort: a failed/looploess emit never breaks the write.
    """
    try:
...
def list_committed_artifacts_by_context_content_type(
    db: StandardDatabase,
    content_type: str,
    *,
    created_by: Optional[str] = None,
) -> List[ArtifactEntity]:
    """Return all COMMITTED artifacts whose ``context.content_type`` matches.

    Used to load platform trust config (issuer artifacts) from the store. When
    ``created_by`` is given, ONLY artifacts created by that principal are returned
    — the trust boundary: a content-type label alone never confers trust (Mantle's
    create is label-blind), only provenance from the platform system principal.
    """
    parts = [
        f"FOR doc IN {COLLECTION_ARTIFACTS}",
        "  FILTER doc.state == \"committed\"",
        "  LET ctx = IS_STRING(doc.context) ? JSON_PARSE(doc.context) : (doc.context || {})",
        "  FILTER ctx.content_type == @content_type",
    ]
    bind_vars: dict = {"content_type": content_type}
    if created_by:
        parts.append("  FILTER doc.created_by == @created_by")
        bind_vars["created_by"] = created_by
    parts.append("  RETURN doc")
    aql = "\n".join(parts)
    try:
        cursor = db.aql.execute(aql, bind_vars=bind_vars)
        return [from_arango_doc(d, ArtifactEntity) for d in cursor]
    except Exception as e:
        logger.error(
            "list_committed_artifacts_by_context_content_type(%s): %s", content_type, e
        )
        return []
...
def list_collection_artifacts(
    ...
    *,
    ...
    committed version anywhere (published cross-collection link).

    ``draft_workspace_id``: when given, a root with NO same-collection version and
    NO committed version resolves to its DRAFT homed in that workspace. This makes
    a draft *linked* into the collection visible WITHIN that workspace's context —
    e.g. dragging a not-yet-committed artifact into a collection. Drafts are
    workspace-private, so the caller (router) MUST gate this on the caller's read
    access to that workspace; with it omitted, such linked drafts are hidden.
    ...
    """
    archived_filter = "" if include_archived else 'FILTER (chosen.state != "archived")'
    aql = f"""
    ...
    FOR v, e IN 1..1 OUTBOUND cdoc {COLLECTION_EDGES}
      ...
      LET committed_fallback = current == null ? FIRST(
        FOR a IN {COLLECTION_ARTIFACTS}
      ...
      LET draft_fallback = (@draft_ws != null AND current == null AND committed_fallback == null) ? FIRST(
        FOR a IN {COLLECTION_ARTIFACTS}
          FILTER a.root_id == e.root_id AND a.state == "draft" AND a.collection_id == @draft_ws
          SORT a.created_time DESC
          LIMIT 1
          RETURN a
      ) : null
      LET chosen = current != null ? current : (committed_fallback != null ? committed_fallback : draft_fallback)
      FILTER chosen != null
...
def update_artifact(db: StandardDatabase, entity: ArtifactEntity) -> Optional[ArtifactEntity]:
    result = replace_document(db, entity, COLLECTION_ARTIFACTS)
    _emit_artifact_change(entity, "artifact.updated")
    return result
