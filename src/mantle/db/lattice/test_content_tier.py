...
verify-on-pull rules — or a failure mode the earlier tier actually shipped (unbounded
re-cache, silent CDN death, promote-of-unreadable). Run:

    python -m pytest src/mantle/db/lattice/test_content_tier.py -q
"""
...
from mantle.db.lattice import s3_content as s3c                           # noqa: E402
from mantle.db.lattice.content_cache import (CacheCorrupt, CacheMiss,     # noqa: E402
                                             ContentKeyMismatch,
                                             FileContentCache)
from mantle.db.lattice.content_tier import (ContentKeyMissing, RemoteCorrupt,  # noqa: E402
                                            RemoteMiss, TieredContentStore,
                                            content_cipher)
from mantle.db.lattice.s3_content import cache_control_for                # noqa: E402
...
def _collection_key(collection: str) -> bytes:
    ...
    ⛔ AN EARLIER FIXTURE RETURNED ONE KEY FOR EVERY COLLECTION (`key = b"k" * 32`), which made the
    entire cross-collection failure class UNREPRESENTABLE: no test could tell two containment roots
    apart, so the shared-ref destruction was invisible to a green suite. Reported by EREA (first
    external consumer, 39 roots / 491 objects, 2 refs destroyed on read).
...
def test_an_unkeyable_collection_is_no_longer_a_thing(tier, cipher):
    """A collection with no `origin_root` used to be uncacheable — every read a WAN read. §1 removed
    the whole category: the at-rest key does not depend on the collection, so there is nothing left
    to be underivable. This is a capability the fix ADDED, not just a defect it removed."""
...
def test_shared_ref_is_readable_by_every_collection(cache):
    ...
    keying contradicted that — one object, N keys, only one of which opened — and resolved the
    contradiction by DESTROYING the incumbent."""
    plain = b"byte-identical across containment roots (EREA: controller/guidance.md, 4 projects)"
    ref = _ref(plain)
...
def test_repeated_verify_is_idempotent_across_collections(cache):
    """EREA measured the damage COMPOUNDING on read: pass 1 reported 2 corrupt, pass 2 reported 6
    missing — the first pass evicted what the second could no longer find. A verify sweep must be
    idempotent; reading is not a write."""
...
# §1 removed the per-collection key, so these no longer fire on a shared ref. They are NOT
# obsolete: a rotated `content.key`, an unmounted keys volume, or any wiring error still presents
# the wrong key, and `get()` — the one operation callers assume is safe — must not delete on it.
# Induced the way it actually happens in production: a second cache over the same directory whose
# node key differs.
...
def test_legacy_shared_ref_converges_for_every_root(legacy_cache):
    """The migration also HEALS §1's damage pattern: two roots, one ref, written under two different
    legacy keys. Whichever is read first wins the rewrite, and the other then opens it under the
    shared key — so the ref ends up readable by both, which it never was before."""
    cache, legacy = legacy_cache
    plain = b"one ref, two roots, two legacy keys"
    ref = _ref(plain)
    legacy.write(ref, plain, "c2")                       # last writer under the OLD scheme
    assert cache.get(ref, collection="c2") == plain      # opens via c2's legacy key, rekeys
    assert cache.get(ref, collection="c1") == plain      # c1 could NEVER read this before
...
def test_disk_floor_never_evicts_without_a_durability_oracle(cache, monkeypatch, caplog):
    """`_ensure_free` used to unlink blobs with NO durability check, justifying itself as "a blob is
    re-fetchable from the S3 remote or re-ingestable" — while never asking. On a `remote=None` node
    (first-class per `content_tier`: "an air-gapped box, not a degraded one") that deleted the only
    copy. `evict_local` and `evict_for_space` both already required proof; this path disagreed, and
...
def test_deprecated_env_name_is_still_honored(monkeypatch, caplog):
    ...
    and the 2026-07-25 disk-fill incident reintroduced by a rename with no error anywhere."""
    from mantle.db.lattice.content_cache import _min_free_bytes
