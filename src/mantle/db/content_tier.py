"""`TieredContentStore` — one content substrate, tiered: local encrypted CAS plus S3 ciphertext.
...
The local tier is the lattice's own encrypted filesystem cache, so a node needs no external
processes to read its working set.
...
This is a CDN substrate for free: every remote object is (a) ciphertext — zero-knowledge to
...
The air-gap invariant: the remote tier is reached only on a local miss. A node whose working set
is cached answers with the backing unreachable (F3 / P7-S3.8); `remote=None` is a legal,
first-class configuration (an air-gapped box), not a degraded one.

Failure modes are distinct and loud, the same contract as `content_cache.py`:
...
def content_cipher(keys_dir) -> "MultiFernet":
    ...
    decrypt-only fallbacks for peers whose keys diverged — with one deliberate
...
class TieredContentStore:
    ...
    Surface is plaintext and collection-aware, like the cache: `get(ref, collection=…)`. This is
    ...
    def __init__(self, cache: Optional[FileContentCache], remote=None, *,
        ...
        fault, and raising here is cheaper than every read raising something misleading later."""
    ...
    def evict_for_space(self, *, min_free_bytes: int, limit: int = 5000) -> Dict[str, object]:
        ...
        the arbitrary cap the project forbids. Uncalled, nothing is evicted (the honest default).
        ...
        on the deployed filesystems); that gap is flagged rather than silently approximated here
        with mtime, which records population order, not use, and would evict the oldest-ingested
        hot object first — worse than random, dressed as better.
